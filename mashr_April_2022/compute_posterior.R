...
# data.temp = mash_update_data(data.temp,ref=3)
# 
...
#Plotting an effect size plot
posterior_beta_dat <- coloc_beta_posterior[(coloc_beta_posterior$gene=='ENSG00000175354') & (coloc_beta_posterior$leadsnp=='rs80262450'),] %>% select(contains('PosteriorMean')) %>% gather('cond','beta')
posterior_sd_dat <- coloc_beta_posterior[(coloc_beta_posterior$gene=='ENSG00000175354') & (coloc_beta_posterior$leadsnp=='rs80262450'),] %>% select(contains('PosteriorSD')) %>% gather('cond','beta')

#Pairwise sharing
# x = get_pairwise_sharing(m2, factor=0.5, lfsr_thresh = 1); 
# col_fun = colorRamp2(c(0.7, 0.85,1), c("blue", "white", "red")); 
# 
# lgd <- list(col_fun = col_fun, title = "Pairwise\n Sharing",legend_height = unit(6, "cm"))
# 
# 
# Heatmap(x, name = "mat", col = col_fun,heatmap_legend_param = lgd)

lfsr_dat <- get_lfsr(m2) %>% as.data.frame() %>% rename_with(~ sapply(str_split(.x,'-'),'[',1))   %>% select(-Ctrl_6)
...
column_ha <- HeatmapAnnotation(`Proportion \nof re-sQTLs`=anno_barplot(prop$n,ylim=c(0,.15)));
ht <- lfsr_dat %>%
