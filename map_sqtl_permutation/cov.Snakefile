...
rule collectCovars:
    ...
    output:
        cov_file=cov_dir+"{cond}_{PC}_PCs.cov"
    run:
        import pandas as pd
        pop_pc_dat=pd.read_csv(input.pop_pcs_file,sep='\t').rename(columns={'PCs':'id'})
        PC=int(wildcards.PC)
        leafcutter_pc_dat=pd.read_csv(input.leafcutter_pcs_file,sep='\t').iloc[0:PC,:]
        cov_dat=pd.concat([leafcutter_pc_dat,pop_pc_dat],ignore_index=True)
        cov_dat.to_csv(output.cov_file,sep='\t',index=False)
