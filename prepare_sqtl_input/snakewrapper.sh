...
gtf_f_v45="$GENCODE_V45_GTF_FILE_UNCOMPRESSED"
gtf_f_v27="${GENCODE_GTF_FILE_UNCOMPRESSED}"
...
snakemake -j 24 --until all -R JuncBedAnnotate -p --config conds="${conds_py}" chrom="${chrom_py}"  intron_clustering_output_dir="${INTRON_CLUSTERING_OUTPUT_DIR}" prepared_sqtl_input_dir="${PREPARED_SQTL_INPUT_DIR}" PC="${PC}" prepare_sqtl_input_script="${prepare_sqtl_input_script}" rename_samples_script="${rename_samples_script}" gtf_f_v45="${gtf_f_v45}" gtf_f_v27="${gtf_f_v27}" ref_genome="${ref_genome}" --directory "${workdir}" --cluster "rm logs/cluster/prepare_phenotype_{rule}/{rule}.{wildcards}.out; rm logs/cluster/prepare_phenotype_{rule}/{rule}.{wildcards}.err; mkdir -p logs/cluster/prepare_phenotype_{rule}; bsub -R \"rusage[mem={resources.mem_mb}] select[mem>{resources.mem_mb}] span[hosts=1]\" -M {resources.mem_mb} -n {threads} -J \"prepare_phenotype_{rule}.{wildcards}\" -G ${group} -o logs/cluster/prepare_phenotype_{rule}/{rule}.{wildcards}.out -e logs/cluster/prepare_phenotype_{rule}/{rule}.{wildcards}.err" -s pheno.Snakefile
