...
dat.loc[:,remaining_samples_cols]=dat.loc[:,remaining_samples_cols].map(lambda elem: 0 if elem == '0/0' else float(Fraction(elem)) )
...
dat=dat.groupby(['chr','s','e','clu','intron'])['intron_usage'].agg('mean').reset_index()
print(dat)
