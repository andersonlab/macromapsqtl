...
rnaseq_samples=""
rnaseq_samples=$(python -c "import pandas as pd; gte_dat=pd.read_csv('${GTE_MAP}',sep='\t',header=None); genotype_samples='${genotype_samples[*]}'.split(' '); rnaseq_samples=gte_dat.loc[ (gte_dat[1].isin(genotype_samples)),0].tolist(); print(' '.join(rnaseq_samples))")
...
for bam_file in "${bam_files[@]}"; do
  f_no_extension="${bam_file%%.bam}";
  f_no_extension="${f_no_extension##*/}"
  sliced_f="$sliced_bams_dir""$f_no_extension"."$snp"_"$gene_chr"_"$gene_start"_"$gene_end".bam
...
rm $sliced_bam_files
rm $sliced_bai_files
printf "Merged BAM files...\n"
samtools index $merged_bam_file
printf "Indexed BAM files...\n"
...
bedGraphToBigWig $averaged_coverage_bedgraph_file $CHROM_SIZES_HG38 $averaged_coverage_bw_file
